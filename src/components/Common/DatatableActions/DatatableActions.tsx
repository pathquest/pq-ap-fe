...
const Actions: React.FC<ActionsProps> = ({ actions, id, optionalData, accountingTool, handleClick, actionRowId, menuClassName }) => {
    ...
    <Dropdown>
      ...
      <Menu className={`!z-[6] !-ml-4 !-mt-2 !w-[${menuClassName ? menuClassName : '140px'}]`}>
        {actions.map((action: any) => (
          <MenuItem key={action} className='laptop:!h-full laptopMd:!h-full lg:!h-full xl:!h-full hd:!h-[44px] 2xl:!h-[44px] 3xl:!h-[44px] !outline-black !px-[15px]' onClick={() => { handleClick(action, id, optionalData) }}>
