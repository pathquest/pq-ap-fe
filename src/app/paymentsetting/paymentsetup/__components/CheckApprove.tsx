...
const CheckApprove: React.FC<DrawerProps> = ({ onClose, Mode, accountId, paymentMethodSetupId }) => {
    ...
    const [isCheckImageVisible, setIsCheckImageVisible] = useState<boolean>(false)
    const [checkImageErrorMessage, setCheckImageErrorMessage] = useState<string>("")
                ...
                <div className='sticky top-0 z-[6] px-5 flex h-[66px] w-full items-center justify-between bg-whiteSmoke'>
                    ...
                    <div className={`flex items-center gap-5 ${Mode === "Approve" ? "" : "hidden"}`}>
                        <Button
                            ...
                            className={`btn-sm !h-9 !bg-transparent rounded-full ${isRejectLoading && 'pointer-events-none opacity-80'} ${isButtonDisable && 'pointer-events-none opacity-80'}`}
                            variant={isButtonDisable ? "btn-outline" : 'btn-outline-error'}>
                        ...
                        <Button
                            ...
                            className={`btn-sm !h-9 rounded-full ${isApproveLoading && 'pointer-events-none opacity-80'} ${isButtonDisable && 'pointer-events-none opacity-80'}`}
                            variant={isButtonDisable ? "btn" : 'btn-primary'}>
