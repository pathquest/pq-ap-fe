...
import { getFavoriteStar } from '@/store/features/reports/reportsSlice'
import { Toast } from 'pq-ap-lib'
import React, { useEffect, useState } from 'react'

const ReportStar = ({ data, isStarClick, arrayData, isLoading }) => {
  const dispatch = useAppDispatch()
  const [starred, setStarred] = useState(false)
  const [clickCount, setClickCount] = useState(0)

  useEffect(() => {
    setStarred(data?.IsFavorite)
    const favoriteCount = arrayData.reduce((count, report) => {
      return count + (report.IsFavorite ? 1 : 0)
    }, 0)
    setClickCount(favoriteCount)
  }, [data, arrayData])

  const handleStarClick = async () => {
    const newStarredStatus = !starred

    // Check if the maximum number of stars (3) has already been marked
    if (newStarredStatus && clickCount === 3) {
      Toast.error('Only 3 reports can be marked as favorite')
      return
    }
    setStarred(newStarredStatus)
    const id = data.Id
    await getData(newStarredStatus, id)
    isStarClick(true)
  }

  const getData = (newStarredStatus, StarId) => {
    const params = {
      Id: Number(StarId),
      IsFavorite: newStarredStatus,
    }
    performApiAction(dispatch, getFavoriteStar, params, (responseData) => {
      setStarred(newStarredStatus)
      isStarClick(true)
      setIsLoading(false)
    })
  ...
  return (
    <button disabled={isLoading}>
      <svg width='20' height='20' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg' onClick={handleStarClick}>
        <path
